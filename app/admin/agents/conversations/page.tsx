...
import { Plus, Copy, Check, Calendar, X, Send } from "lucide-react";
import { type Conversation, type ConversationStatus, type Lead } from "@/lib/supabase";
...
export default function ConversationsPage() {
  ...
  async function fetchData() {
    setLoading(true);
    const [convData, leadsData] = await Promise.all([
      fetch('/api/admin/db/conversations?select=*,lead:leads(*)&order=updated_at.desc').then(r => r.json()),
      fetch('/api/admin/db/leads?select=id,business_name,city,google_rating,review_count&status=contacted&order=business_name.asc').then(r => r.json()),
    ]);
    if (Array.isArray(convData)) {
      setConversations(convData as Conversation[]);
      if (convData.length > 0) setSelected(convData[0] as Conversation);
    }
    if (Array.isArray(leadsData)) setLeads(leadsData as Lead[]);
    setLoading(false);
  }
...
Write ONLY the response message. No preamble, no explanation. Just the message itself.`,
  ...
  async function createConversation() {
    ...
    const res = await fetch('/api/admin/db/conversations', {
      method: 'POST',
      headers: {'Content-Type': 'application/json'},
    ...
    const data = await res.json();
    if (data?.id) {
      // Re-fetch to get lead join data
      const fullRes = await fetch(`/api/admin/db/conversations?select=*,lead:leads(*)&id=${data.id}`);
      const fullData = await fullRes.json();
      const conv = Array.isArray(fullData) && fullData[0] ? fullData[0] : data;
      setConversations((prev) => [conv as Conversation, ...prev]);
      setSelected(conv as Conversation);
    }
    setNewReplyText("");
    setNewLeadId("");
  ...
  async function updateStatus(status: ConversationStatus) {
    if (!selected) return;
    await fetch(`/api/admin/db/conversations?id=${selected.id}`, {
      method: 'PATCH', headers: {'Content-Type': 'application/json'},
      body: JSON.stringify({ status, updated_at: new Date().toISOString() }),
    });
    if (status === "demo_booked" && selected.lead_id) {
      await fetch(`/api/admin/db/leads?id=${selected.lead_id}`, {
        method: 'PATCH', headers: {'Content-Type': 'application/json'},
        body: JSON.stringify({ status: "demo_booked", updated_at: new Date().toISOString() }),
      });
    }
