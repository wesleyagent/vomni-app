...
function Toast({ message }: { message: string }) {
  return (
    <div className="animate-fade-in mt-3 flex items-center gap-2 rounded-lg bg-green-50 border border-green-200 px-4 py-2 text-sm font-medium text-green-700">
      <Check size={16} />
      {message}
    </div>
  );
...
export default function SettingsPage() {
  ...
  useEffect(() => {
    ...
    setSmsTemplate(biz.smsTemplate);
  ...
  const smsPreview = smsTemplate
    .replace(/\{name\}/g, "John")
    .replace(/\{link\}/g, "vomni.app/r/abc");
      ...
      <div className="space-y-6 max-w-3xl">
        ...
        >
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div>
              ...
              <input
                ...
                className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm text-gray-900 focus:border-primary-500 focus:ring-1 focus:ring-primary-500 outline-none"
              />
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1">
          ...
          <div className="mt-4 flex justify-end">
            <SaveButton
              onClick={() => save({ name, ownerName, phone, address })}
            />
          </div>
        </SectionCard>
        ...
        <SectionCard
          ...
          title="SMS Template"
          description="Customize the review request message"
        >
          <textarea
            value={smsTemplate}
            onChange={(e) => setSmsTemplate(e.target.value)}
            rows={4}
            className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm text-gray-900 focus:border-primary-500 focus:ring-1 focus:ring-primary-500 outline-none resize-none"
          />
          <div className="mt-1.5 flex items-center justify-between">
            <p className="text-xs text-gray-400">
              Use {"{name}"} for customer name and {"{link}"} for review link
            </p>
            <span
              className={`text-xs font-medium ${
                smsTemplate.length > 160 ? "text-orange-500" : "text-gray-400"
              }`}
            >
          ...
          <div className="mt-4 rounded-lg bg-gray-50 border border-gray-100 p-4">
            ...
            <div className="rounded-lg bg-green-100 px-4 py-3 text-sm text-gray-800 max-w-xs">
              {smsPreview}
            </div>
          </div>

          <div className="mt-4 flex justify-end">
            <SaveButton onClick={() => save({ smsTemplate })} />
          </div>
        </SectionCard>
        ...
        >
          <div className="space-y-5">
            <div>
              ...
              <input
                ...
                className="w-32 rounded-lg border border-gray-200 px-3 py-2 text-sm text-gray-900 focus:border-primary-500 focus:ring-1 focus:ring-primary-500 outline-none"
              />
              <p className="mt-1.5 text-xs text-gray-400">
          ...
          <div className="mt-4 flex justify-end">
            <SaveButton
              onClick={() =>
                save({ reviewRequestDelay, quietHoursStart, quietHoursEnd })
              }
            />
          </div>
        </SectionCard>
        ...
        >
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-gray-400">
                  Get an email when a customer leaves a low rating
                </p>
              </div>
              <button
                role="switch"
                ...
                className={`relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out ${
                  notifyOnNegative ? "bg-primary-600" : "bg-gray-200"
                }`}
              >
                <span
                  className={`pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow-sm ring-0 transition duration-200 ease-in-out ${
                    notifyOnNegative ? "translate-x-5" : "translate-x-0"
                  }`}
                />
              </button>
            </div>
          ...
          <div className="mt-4 flex justify-end">
            <SaveButton
              onClick={() => save({ notifyOnNegative, notifyEmail })}
            />
          </div>
        </SectionCard>
        ...
        <SectionCard
          ...
          title="Brand Color"
          description="Customize the look of your review pages"
        >
          ...
          <div className="mt-4 flex justify-end">
            <SaveButton onClick={() => save({ brandColor })} />
          </div>
        </SectionCard>
      </div>
    </div>
